...
void swapparticleinfo(int *a, int *b, double prob)
{
	...
	while(flag)
	{
		...
		for(int i = 0;i < (SIZE);)
		{
			...
			/*for(int j = (i* (BIT_LENGTH+SIZE)); j < ((i* (BIT_LENGTH+SIZE)) + BIT_LENGTH); j++)	
				{
					if(drand48() <= prob)
						a[j] = b[j];
				}
			binary = 0;	
			for(int k = 0; k < BIT_LENGTH ; k++)
				binary = binary + a[(i* (BIT_LENGTH+SIZE)) + k] * pow (10,BIT_LENGTH - 1 -k);*/
			// power_decimal	= bin_to_decimal(binary);
...
void initialiseparticle(particle *object)
{
	...
	while(flag)
	{
		...
		for(int i = 0;i < (SIZE);)
		{
			...
			binary = 0;	
			for(int k = 0; k < BIT_LENGTH ; k++)
				binary = binary + object->info[(i* (BIT_LENGTH+SIZE)) + k] * pow (10,BIT_LENGTH - 1 -k);*/
			// printf("%d %d %d\n", decimal, power_decimal, tam_testtime[i].power_testtime_list[decimal].no_of_power);

			

			if(decimal< tam_testtime[i]. no_of_tam )//&& power_decimal < tam_testtime[i].power_testtime_list[decimal].no_of_power )
			{
				// printf("%d %d\n", decimal, tam_testtime[i].power_testtime_list[decimal].no_of_power);
